...
st.title("DETEKSI PENYAKIT MATA PADA SAPI")

#st.sidebar.markdown("## Choose detection type:")
model_type = st.sidebar.radio("", ("SSD", "YOLOv4"))
...
if model_type == "YOLOv4":
    start = time.time()
    model_info = load_model("configs/yolov4-tiny-custom.cfg", "weights/yolov4-tiny.weights", "coco.names")
    end = time.time()
...
st.sidebar.markdown("## Model time load:")
...
if img_stream:
    ...
    if model_type == "pink-eye detection":
        st.write("Found ", classIDs.count(0), " pink-eye and ", classIDs.count(1), " sehat")
    else:
